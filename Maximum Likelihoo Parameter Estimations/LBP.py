...
class LBP:
    ...
    def belief_propagation(self, its):
        iteration = 0
        while(iteration<its):
...
def unnorm_prob(obs, algo):
    p = 1
    for i in range(algo.N):
        p = p*algo.marginal_node(i,obs[i])
        adjacent_vertices = np.nonzero(algo.adj_matrix[i])[0]
        for j in adjacent_vertices[adjacent_vertices>i]:
            p=p*algo.marginal_edge(i,j,obs[i],obs[j])
    return p




def conditional_marginals(A, w, its, obs, V_m, X_m):
    __w = deepcopy(w)
    ...
    def phi_function(i, x_i):
        if i in obs.keys():
            return math.exp(__w[x_i])*(obs[i]==x_i)
        else:
            return math.exp(__w[x_i])

    algo = LBP(A, len(w), psi=psi_function, phi=phi_function, sum=True)
    algo.belief_propagation(its)
    q = []
    for x in X_m:
        q = q + [unnorm_prob({**obs,**dict(zip(V_m,x))},algo)]
    if np.sum(q):
        q = q/np.sum(q)
    return q


def sum_of_marginals(A, w, its):
    __w = deepcopy(w)
    ...
    algo = LBP(A, len(w), psi=psi_function, phi=phi_function, sum=True)
    algo.belief_propagation(its)
    marginals = np.zeros(algo.domain_size)
    for i in range(algo.N):
        adjacent_vertices = np.nonzero(algo.adj_matrix[i])[0]
        tmp = list()
        for x in range(algo.domain_size):
            tmp.append(algo.phi_function(i, x) * np.prod(algo.message_clique_to_vertex[x, adjacent_vertices, i]))
        if np.sum(tmp):
            tmp = tmp / np.sum(tmp)
        marginals = marginals + tmp
    return marginals
